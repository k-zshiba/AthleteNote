...
if (isset($_REQUEST['oauth_token']) && $twitter_request_token['oauth_token'] !== $_REQUEST['oauth_token']) {
    echo 'Twitterの認証に失敗しました。<br>'.'<button type="button" class="btn btn-danger" onclick="location.href=\'./topPage.php\'">トップに戻る</button>';
    exit;
}else if (isset($_REQUEST['oauth_token']) && $twitter_request_token['oauth_token'] === $_REQUEST['oauth_token']) {
    ...
    $access_token = $twitter_connection->oauth("oauth/access_token", ["oauth_verifier" => $_REQUEST['oauth_verifier']]);
